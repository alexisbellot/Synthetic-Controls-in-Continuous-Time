...
def get_smoking_data(selected_state=2):
    ...
    data = pd.read_csv('smoking.csv', header = 0)
    ...
    y = np.array(y)[:,:,1].astype(float) / 100.


    t = torch.linspace(0., 1., y.shape[1])
...
def get_emu_data(selected_country=4):
    ...
    data = pd.read_csv('hope_emu.txt', header = 0)
    ...
    y = np.array(y)[:,:,1].astype(float) / 10.    
...
def simulate_statins(T, a, delta_t=0.1, sd=0.01, burn_in=1000,
                       seed=None):
    if seed is not None:
        np.random.seed(seed)
